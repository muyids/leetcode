...
function getDir(num) {
  return (parseInt((num - 1) / 100) * 100 + 1) + '-' + (100 * (parseInt((num - 1) / 100) + 1))
}
...
LeetSolver.prototype.printLink = function (num) {
  ...
  const link = `- [LeetCode ${id}. ${problem.stat.question__title} (${LeetSolver.LEVEL[problem.difficulty.level]})](./problems/${getDir(id)}/${id}.${problem.stat.question__title_slug}.md)`
  console.log(link)
}
